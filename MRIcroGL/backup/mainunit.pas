...
private
  MRIScans: TStringList;
  PETScans: TStringList;
  DWIScans: TStringList;
  OTHERScans: TStringList;

  CurrentScanType: string;

  procedure LoadScanGroup(scanType: string);
  procedure RefreshLayers(scanType: string);

  procedure btnMRIClick(Sender: TObject);
  procedure btnPETClick(Sender: TObject);
  procedure btnDWIClick(Sender: TObject);
...
begin
   ...
   if not OpenDialog1.execute then
      OpenDialog1.filename := '';
    if Pos('pet', LowerCase(OpenDialog1.filename )) > 0 then
      PETScans.Add(OpenDialog1.filename )
    else if Pos('dwi', LowerCase(OpenDialog1.filename )) > 0 then
      DWIScans.Add(OpenDialog1.filename )
    else if Pos('mri', LowerCase(OpenDialog1.filename )) > 0 then
      MRIScans.Add(OpenDialog1.filename )
    else
      OTHERScans.Add(OpenDialog1.filename );
